...
def worker_fn(rank: int, world_size: int, args):
    print(f"Rank {rank}: Starting worker (Overlapping DDP).")
    ...
    base_model = model_class(**model_config).to(device)


    ddp_model = DDP(base_model)
    print(f"Rank {rank}: DDP model wrapped and initial weights broadcasted from rank 0.")


    if rank == 0:
        torch.manual_seed(rank + 42)
        non_parallel_model = model_class(**model_config).to(device)
    else:
        non_parallel_model = None
    ...
    if rank != 0:
        ...
        for p_ddp, p_rank0_init in zip(ddp_model.module.parameters(), rank0_initial_model.parameters()):
        ...
        print(f"Rank {rank}: Initial parameter verification passed.")
    dist.barrier()
    ...
    print(f"Rank {rank}: Starting training loop for {args.num_warmup_steps} warmup + {args.num_measurement_steps} measurement steps (Overlapping DDP).")
    ...
    print(f"Rank {rank}: Training finished (Overlapping DDP).")
    ...
    if rank == 0:
        ...
        print("=== Timing Summary (Overlapping DDP) ===")
...
def main():
    ...
    parser.add_argument('--num_measurement_steps', type=int, default=5)
    args = parser.parse_args()

    print(f"Starting Overlapping DDP (Individual Parameters) with world_size={args.world_size}")
    mp.spawn(worker_fn,
